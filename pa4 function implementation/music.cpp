...
void readToVect(vector<Record> vec, int size, ifstream& fileHandler)
{
    ...
    int i = 0;
    size = vec.size();
    while (!fileHandler.eof() && i < size)
    {
        getline(fileHandler, vec[i].artist, delim1);
        getline(fileHandler, vec[i].albumTitle,delim1);
        getline(fileHandler, vec[i].songTitle, delim1);
        getline(fileHandler, vec[i].genre, delim1);
        //fileHandler >> vec[i].(Duration Minutes);
        //fileHandler >> vec[i].(Duration seconds) , Delim 2;
        fileHandler >> vec[i].playCount >> delim1;
        fileHandler >> vec[i].Rating >> delim1;
    ...
    // for (int i = 0; i < size; ++i)
     cout <<"\n" << vec[i].artist << ' '<< vec[i].songTitle << ' ' << vec[i].playCount << ' ';
}
